...
// @updateURL    https://modulah.github.io/landlord-tenant-ledger/MoDuLs-Landlord-Tenant-Ledger.user.js
// @downloadURL  https://modulah.github.io/landlord-tenant-ledger/MoDuLs-Landlord-Tenant-Ledger.user.js
...
(function () {
  ...
  const HOSTED_API_BASE = "https://pp_api.sokin.xyz/pit-guru";
  ...
  const ENDPOINTS = [
    ...
    { id: "user-properties", path: "/user/properties", label: "Homes I own", detail: "Find homes owned by your API-key user.", action: "import-owned", actionLabel: "Fetch my owned homes" },
  ...
  function apiGet(path, params = {}) {
    ...
    if (!key) {
      return Promise.reject(new Error("Add your Torn public API key first."));
    }
      ...
      GM_xmlhttpRequest({
        ...
        onload(response) {
          try {
            ...
            if (body.error) {
              reject(new Error(body.error.error || body.error.message || "Torn API error"));
              return;
            }
            if (response.status >= 400) {
              reject(new Error(`Torn API returned HTTP ${response.status}`));
              return;
            }
            resolve(body);
          } catch (_error) {
        ...
        onerror() {
          reject(new Error("Could not reach the Torn API."));
        },
      });
    });
  }
      ...
      GM_xmlhttpRequest({
        ...
        onload(response) {
          ...
          if (response.status < 200 || response.status >= 300) {
            const error = new Error(body.error || body.message || `Hosted ledger returned HTTP ${response.status}`);
        ...
        onerror() {
          const error = new Error("Could not reach the hosted ledger server.");
          error.network = true;
          reject(error);
        },
        ontimeout() {
          const error = new Error("Hosted ledger server timed out.");
          error.network = true;
          reject(error);
        },
      });
  ...
  async function runHostedFirst(scope, fallback) {
    try {
      setStatus("Asking the subscription server to fill the ledger...");
      return await syncHostedLedger(scope);
    } catch (error) {
      if (!canUseDirectTornFallback(error)) throw error;
      setStatus(`Server sync is not ready (${error.message || "unknown error"}). Fetching directly from Torn for now...`);
  ...
  function applyHostedLedgerSync(body, scope = "all") {
    if (!body || typeof body !== "object") throw new Error("Hosted ledger server returned an empty response.");
    ...
    if (partnerProperties.length) storeEndpointDataWithoutRender("spouse-properties", { properties: partnerProperties }, `Server returned ${partnerProperties.length.toLocaleString()} partner homes/contracts.`);
    ...
    state.endpointData = {
      ...
      "server-insights": {
        ...
        fetchedAt: nowIso(),
        message: suggestions.length ? `Server returned ${suggestions.length.toLocaleString()} rent suggestions.` : "Server sync did not include rent suggestions yet.",
      },
    };
  ...
  async function checkApiKey() {
    ...
    if (!userId) {
      saveState();
      render();
      throw new Error(`The key info endpoint did not return a user ID in a known shape. Top-level keys: ${Object.keys(body || {}).join(", ") || "none"}.`);
    }
    ...
    saveState();
    render();
    setStatus(`API key checked. User ID ${userId} is ready for user-specific calls.`);
    return body;
  }
  ...
  async function fetchMarketProperties() {
    throw new Error("Market checks now run on the subscription server. Use Fetch everything.");
  }

  async function fetchMarketRentals() {
    throw new Error("Rental checks now run on the subscription server. Use Fetch everything.");
  }

  async function scanRentalRoi() {
    throw new Error("ROI scans now run on the subscription server. Use Fetch everything.");
  }
  ...
  function saveApiKeyFromInput(input) {
    ...
    if (!value) throw new Error("Paste a Torn public API key first.");
    state.apiKey = value;
    state.keyInfo = null;
    clearHostedSession();
    ...
    saveState();
    render();
    setStatus(`Saved API key ending ${value.slice(-4)}.`);
  }
  ...
  function updateLease(id, patch, changedProp = "") {
    state.leases = state.leases.map((lease) => (
      lease.id === id ? recalculateLease({ ...lease, ...patch, updatedAt: nowIso() }, changedProp) : lease
    ));
    saveState();
    render();
  }
  ...
  function syncManualLeaseForm(form, changedProp) {
    ...
    const lease = recalculateLease({
      ...
      remainingDays: positiveNumber(form.elements.remainingDays && form.elements.remainingDays.value),
    ...
    if (form.elements.remainingDays && lease.remainingDays !== "") form.elements.remainingDays.value = lease.remainingDays;
  ...
  function exportRentSuggestionsCsv() {
    ...
    if (!rows.length) throw new Error("Fetch everything first. Suggestions are created by the server.");
      ...
      <div class="tlt-panel" role="region" aria-label="Torn lease ledger">
        ...
        <div class="tlt-foot">
          <div>
            ...
            <div class="tlt-muted">${keyInfoHtml()}${serverAccessHtml()} Fetching uses the subscription server first; direct Torn fetch is only a fallback while the server route is unavailable.</div>
          </div>
    ...
    return `
      <div class="tlt-panel-grid">
        <div class="tlt-endpoint">
          ...
          <span class="tlt-muted">Save your key first. Then fetch once and let the server fill the list.</span>
    ...
    return `
      <div class="tlt-settings-grid">
        <div class="tlt-settings-card">
          ...
          <span class="tlt-muted">Paste a public Torn API key. It is used to check access and fetch your homes.</span>
          <label>
            Torn public API key
        ...
        <div class="tlt-settings-card">
          ...
          <label class="tlt-setting-row">
            ...
            <span>Show raw API text below tables</span>
          </label>
          <label class="tlt-setting-row">
        ...
        <div class="tlt-settings-card">
          ...
          <span class="tlt-muted">Sent to the server so suggestions use the target you want.</span>
          <label>
            Target yearly ROI %
  ...
  function insightsHtml() {
    ...
    return `
      <div class="tlt-panel-grid">
        ...
        <div>
          ${suggestions.length ? dataTableHtml({ suggestions }, "rent-suggestions") : `<div class="tlt-muted">No server suggestions yet. Press Fetch everything.</div>`}
        </div>
      </div>
  ...
  function advancedHtml() {
    ...
    return `
      <div class="tlt-guide">
        ...
        <span class="tlt-muted">Most people can ignore this page. It is here when you want to inspect a specific Torn property API response.</span>
      </div>
  ...
  function rentSuggestionsHtml() {
    ...
    return `
      ...
      ${rows.length ? dataTableHtml({ suggestions: rows }, "rent-suggestions") : `<div class="tlt-muted">No server suggestions yet. Press Fetch everything.</div>`}
    `;
  }
  ...
  function fillManualFormFromOwnedProperty(form) {
    ...
    setValue("tenant", option.dataset.tenant);
    ...
    setValue("remainingDays", option.dataset.remainingDays);
    ...
    return `
      <div class="tlt-guide">
        ...
        <div class="tlt-steps">
          ...
          <div class="tlt-step">
            <b>2. Fetch homes</b>
            <span>Let the server fill in owned and current homes.</span>
  ...
  function ledgerHtml(leases, summary) {
    ...
    return `
      ...
      <form class="tlt-form" data-form="manual">
        ...
        <label class="tlt-form-field"><span>Renter</span><input name="tenant" list="tlt-tenant-options" placeholder="Name or ID"><datalist id="tlt-tenant-options">${manualTenantOptionsHtml()}</datalist></label>
        ...
        <label class="tlt-form-field"><span>Days left</span><input name="remainingDays" type="number" min="0" step="1" placeholder="0"></label>
        <label class="tlt-form-field"><span>Status</span><select name="status">
          ${statusOptionsHtml("rented")}
        </select></label>
        <label class="tlt-form-field"><span>Show under</span><select name="roleHint">
          <option value="landlord"${state.role === "landlord" ? " selected" : ""}>Homes I rent out</option>
          <option value="tenant"${state.role === "tenant" || state.role === "all" ? " selected" : ""}>Homes I use</option>
        </select></label>
  ...
  function rowHtml(lease) {
    return `
      <tr data-id="${escapeAttr(lease.id)}">
        ...
        <td><input class="tlt-lease-input tlt-lease-text" data-prop="landlord" style="${inputWidthStyle(lease.landlord, 10)}" value="${escapeAttr(lease.landlord)}"></td>
        ...
        <td><input class="tlt-lease-input" data-prop="remainingDays" type="number" min="0" step="1" style="${inputWidthStyle(lease.remainingDays, 6)}" value="${escapeAttr(lease.remainingDays)}"></td>
        <td>
  ...
  function bind(root, isPopup = false) {
    ...
    if (manualForm) {
      manualForm.querySelectorAll("input[name='amount'], input[name='dailyAmount'], input[name='durationDays'], input[name='startDate'], input[name='endDate'], input[name='remainingDays']").forEach((field) => {
    ...
    root.querySelectorAll("tbody [data-prop]").forEach((field) => {
      ...
      field.addEventListener("change", () => {
        ...
        const numeric = ["durationDays", "remainingDays"].includes(prop);
      ...
      button.addEventListener("click", async (event) => {
        ...
        try {
          ...
          if (action === "clear-data") {
            if (confirmAction(root, "Delete all saved leases, settings, and the saved API key?")) {
              state.leases = [];
