...
(function () {
   ...
   * ============================ */
    ...
    let driverHistory = {};
    let driverIntelCacheLoaded = false;
    let driverHistoryLoaded = false;
    let recordsLoaded = false;
    let deferredStartupCachesScheduled = false;
    let driverHistoryIndexDirty = true;
    let recordsIndexDirty = true;
    let driverHistoryByDriverIndex = new Map();
    let recordsByDriverIndex = new Map();
    ...
    let preRaceSummaryStats = newPreRaceSummaryStats_();

    function newPreRaceSummaryStats_() {
        return {
            captures: 0,
            changes: 0,
            workerParses: 0,
    ...
    let preRaceSummaryWorker = null;
    let preRaceSummaryWorkerSequence = 0;
    let preRaceSummaryAppliedSequence = 0;
    const preRaceSummaryWorkerPending = new Map();
    ...
    let directRacingDataParticipantsById = new Map();
    ...
    let aggregateWorkerFailedKey = "";
    let aggregateWorkerLastStatus = "";
    let aggregateWorkerLastError = "";
    let aggregateWorkerLastMs = 0;
   ...
   * ============================ */
    ...
    function buildPreRaceSummaryWorkerSource_() {
        return `"use strict";
const hasTiming=race=>{
  const cars=race&&race.cars&&typeof race.cars==="object"?race.cars:{};
  for(const encoded of Object.values(cars)){
    if(typeof encoded!=="string"||encoded.length<4)continue;
    try{
      const values=atob(encoded).split(",");
      let positive=0;
      for(const value of values){if(Number(value)>0&&++positive>1)return true;}
    }catch{}
  }
  return false;
};
const summarize=(root,includeParticipants=true)=>{
  ...
  if(includeParticipants){
    ...
    for(const [key,value,index] of entries){
      ...
      participants.push({
        ...
        itemID:row.itemID||row.itemId||row.item_id||row.imteID||row.carItemID||row.carItemId||"",
        car:row.carTitle||row.car_title||row.car||"",
        carImg:row.carImage||row.carImg||row.car_image||""
      });
    }
  }
  return {
...
self.onmessage=event=>{const id=event.data&&event.data.id;try{const root=JSON.parse(String(event.data&&event.data.raw||""));self.postMessage({id,ok:true,summary:summarize(root,event.data&&event.data.includeParticipants!==false)});}catch(error){self.postMessage({id,ok:false,error:String(error&&error.message||error||"Pre-race summary parse failed")});}};`;
    }

    function preRaceSummaryWorker_() {
        if (preRaceSummaryWorker) return preRaceSummaryWorker;
        try {
            if (typeof Worker !== "function" || typeof Blob !== "function" || typeof URL?.createObjectURL !== "function") return null;
            const url = URL.createObjectURL(new Blob([buildPreRaceSummaryWorkerSource_()], { type: "text/javascript" }));
            preRaceSummaryWorker = new Worker(url);
            URL.revokeObjectURL(url);
            preRaceSummaryWorker.onmessage = event => {
                const message = event?.data || {};
                const pending = preRaceSummaryWorkerPending.get(message.id);
                if (!pending) return;
                preRaceSummaryWorkerPending.delete(message.id);
                if (message.ok) pending.resolve(message.summary || {});
                else pending.reject(new Error(message.error || "Pre-race summary parse failed"));
            };
            preRaceSummaryWorker.onerror = () => {
                for (const pending of preRaceSummaryWorkerPending.values()) pending.reject(new Error("Pre-race summary worker failed"));
                preRaceSummaryWorkerPending.clear();
                try { preRaceSummaryWorker?.terminate(); } catch {}
                preRaceSummaryWorker = null;
            };
            return preRaceSummaryWorker;
        } catch {
            preRaceSummaryWorker = null;
            return null;
        }
    ...
    function preRaceSummaryFromObject_(raw, includeParticipants = true) {
        ...
        const preRace = Number.isFinite(started) && Number.isFinite(current)
            ? current < started
    ...
    async function parsePreRaceSummaryOffMain_(raw, includeParticipants = true) {
        const worker = preRaceSummaryWorker_();
        if (!worker) {
            await yieldToMain_();
            return preRaceSummaryFromObject_(JSON.parse(String(raw || "")), includeParticipants);
        }
        const id = ++preRaceSummaryWorkerSequence;
        preRaceSummaryStats.workerParses += 1;
        return await new Promise((resolve, reject) => {
            preRaceSummaryWorkerPending.set(id, { resolve, reject });
            worker.postMessage({ id, raw: String(raw || ""), includeParticipants });
        });
    }
    ...
    function invalidateDriverHistoryIndex_() {
        driverHistoryIndexDirty = true;
        driverHistoryByDriverIndex = new Map();
    }

    function invalidateRecordsIndex_() {
        recordsIndexDirty = true;
        recordsByDriverIndex = new Map();
    }

    function ensureRecordsLoaded_(reason = "demand") {
    ...
    function ensureDriverIntelCacheLoaded_(reason = "demand") {
        if (driverIntelCacheLoaded) return true;
        driverIntelCache = perfMeasure_("startup.loadDriverIntelCache", () => loadDriverIntelCache_(), { reason }) || {};
        driverIntelCacheLoaded = true;
        return true;
    }

    function ensureDriverHistoryLoaded_(reason = "demand") {
        if (driverHistoryLoaded) return true;
        driverHistory = perfMeasure_("startup.loadDriverHistory", () => loadDriverHistory_(), { reason }) || {};
    ...
    function perfRecord_(name, ms, meta = {}) {
        if (!PHASE_REWRITE_FLAGS.instrumentation || !name || !Number.isFinite(Number(ms))) return;
        const key = String(name);
        const value = Math.max(0, Number(ms) || 0);
        const stat = phasePerfStats[key] || {
            count: 0,
            totalMs: 0,
            maxMs: 0,
            lastMs: 0,
        ...
        if (value >= PERF_SLOW_MS) stat.slow += 1;
        const sample = {
            seq: ++phasePerfSequence,
            at: stat.lastAt,
            name: key,
            ms: Math.round(value * 100) / 100,
            meta: meta && typeof meta === "object" ? meta : {}
        };
        stat.samples.push(sample);
        if (stat.samples.length > PERF_SAMPLE_MAX) stat.samples.splice(0, stat.samples.length - PERF_SAMPLE_MAX);
        phasePerfStats[key] = stat;
        if (debugEnabled && value >= PERF_SLOW_MS) {
            console.debug(TAG, "perf", key, `${sample.ms}ms`, sample.meta);
        }
    }

    function perfMeasure_(name, fn, meta = {}) {
    ...
    function perfStatsSnapshot_() {
        const out = {};
        for (const [name, stat] of Object.entries(phasePerfStats)) {
            out[name] = {
                count: stat.count || 0,
                avgMs: stat.count ? Math.round((stat.totalMs / stat.count) * 100) / 100 : 0,
                maxMs: Math.round((stat.maxMs || 0) * 100) / 100,
                lastMs: Math.round((stat.lastMs || 0) * 100) / 100,
                slow: stat.slow || 0,
                lastAt: stat.lastAt || 0,
                samples: (stat.samples || []).slice(-10)
            };
        }
        return out;
    }
    ...
    async function clearPerformanceTestCaches_() {
        ...
        directRacingDataParticipantsById = new Map();
        directRacingDataParticipantsByName = new Map();
        ...
        preRaceSummaryStats = newPreRaceSummaryStats_();
        preRaceSummaryReceivedPerfMs = 0;
        for (const pending of preRaceSummaryWorkerPending.values()) pending.reject(new Error("Pre-race summary cache reset"));
        preRaceSummaryWorkerPending.clear();
        try { preRaceSummaryWorker?.terminate(); } catch {}
        preRaceSummaryWorker = null;
        preRaceSummaryWorkerSequence = 0;
        preRaceSummaryAppliedSequence = 0;
    ...
    function boundedMapGet_(map, key) {
        if (!map || !key || !map.has(key)) return undefined;
    ...
    function trimMapToMax_(map, maxEntries, keepKey = "") {
        if (!map) return;
        const max = Math.max(1, Number(maxEntries) || 1);
        const keep = String(keepKey || "");
        if (keep && map.has(keep)) {
            const value = map.get(keep);
            map.delete(keep);
            map.set(keep, value);
        }
        while (map.size > max) {
            const oldest = map.keys().next().value;
            if (oldest === undefined) break;
            if (keep && oldest === keep && map.size > 1) {
                const value = map.get(oldest);
                map.delete(oldest);
                map.set(oldest, value);
                continue;
            }
            map.delete(oldest);
        }
    ...
    function trimRaceDataCaches_(activeRaceId = "") {
        const activeKey = activeRaceId ? raceDataFetchRaceKey_(activeRaceId) : "";
        trimMapToMax_(raceDataPayloadCacheByRaceId, RACE_DATA_PAYLOAD_CACHE_MAX, activeKey);
        trimMapToMax_(raceDataAcceptedStateByRaceId, RACE_DATA_META_CACHE_MAX, activeKey);
    ...
    function cleanupRaceLifecycle_(reason = "maintenance", options = {}) {
        ...
        const before =
              ...
              (driverIntelCacheLoaded ? countObjectKeys_(driverIntelCache) : 0) +
              (driverHistoryLoaded ? countObjectKeys_(driverHistory) : 0);
        ...
        if (driverIntelCacheLoaded) driverIntelCache = pruneDriverIntelCache_(driverIntelCache || {});
        if (driverHistoryLoaded) {
        ...
        lifecycleCleanupStats = {
            ...
            intelEntries: driverIntelCacheLoaded ? countObjectKeys_(driverIntelCache) : 0,
            historyEntries: driverHistoryLoaded ? countObjectKeys_(driverHistory) : 0
        };
        if (options.persist !== false) {
            if (driverIntelCacheLoaded) saveJson_(STORE_DRIVER_INTEL_CACHE_KEY, driverIntelCache || {});
            if (driverHistoryLoaded) saveJson_(STORE_DRIVER_HISTORY_KEY, driverHistory || {});
        }
    ...
    function phaseCacheStats_() {
        return {
            ...
            acceptedStates: raceDataAcceptedStateByRaceId.size,
            fetchStats: raceDataFetchStatsByRaceId.size,
            inFlight: raceDataInFlightByRaceId.size,
            ...
            recordsLoaded,
            recordsEntries: recordsLoaded ? (records || []).length : 0,
            driverIntelLoaded: driverIntelCacheLoaded,
            ...
            preRaceSummary: {
                ...preRaceSummaryStats,
                indexedById: directRacingDataParticipantsById.size,
                indexedByName: directRacingDataParticipantsByName.size,
                workerActive: !!preRaceSummaryWorker
            },
            longTaskStandard: {
            ...
            aggregateWorker: {
                ...
                status: aggregateWorkerLastStatus,
                error: aggregateWorkerLastError,
                lastMs: aggregateWorkerLastMs
            }
        };
    }
    ...
    function saveRecords_() {
        if (!recordsLoaded) {
            const pending = Array.isArray(records) ? records.slice() : [];
            ensureRecordsLoaded_("saveRecords");
            if (pending.length) {
                const byId = new Map((records || []).map(r => [String(r?.id || `${r?.mode || ""}|${r?.track || ""}|${r?.car || ""}|${r?.driverName || ""}|${r?.atIso || ""}`), r]));
                for (const row of pending) {
                    const key = String(row?.id || `${row?.mode || ""}|${row?.track || ""}|${row?.car || ""}|${row?.driverName || ""}|${row?.atIso || ""}`);
    ...
    function saveDriverIntelCache_() {
        if (!driverIntelCacheLoaded) ensureDriverIntelCacheLoaded_("saveDriverIntelCache");
    ...
    function saveDriverHistory_() {
        if (!driverHistoryLoaded) ensureDriverHistoryLoaded_("saveDriverHistory");
        driverHistory = pruneDriverHistory_(driverHistory || {});
        invalidateDriverHistoryIndex_();
   ...
   * ============================ */
    ...
    function resetForRaceChange_(newRaceId) {
        ...
        aggregateWorkerFailedKey = "";
        aggregateWorkerLastStatus = "";
        aggregateWorkerLastError = "";
        aggregateWorkerLastMs = 0;
    ...
    function getCachedDriverIntel_(driverId, maxAgeHours = 24 * 7) {
        if (!driverIntelCacheLoaded) return null;
    ...
    function cacheDriverIntel_(key, intel) {
        if (!intel) return;
        if (!driverIntelCacheLoaded) ensureDriverIntelCacheLoaded_("cacheDriverIntel");
    ...
    function getCachedDriverIntelByName_(name, maxAgeHours = 24 * 7) {
        ...
        if (!nn) return null;
        const values = Object.values(driverIntelCache || {});
    ...
    function persistBigRaceProcessedSummary_(model) {
        ...
        const cached = boundedMapGet_(bigRaceProcessedSummaryCache, identity.key);
        if (cached) {
        ...
        (async () => {
            ...
            if (stored?.summary) {
                boundedMapSet_(bigRaceProcessedSummaryCache, identity.key, stored.summary, BIG_RACE_SUMMARY_MEMORY_MAX);
            ...
            boundedMapSet_(bigRaceProcessedSummaryCache, identity.key, summary, BIG_RACE_SUMMARY_MEMORY_MAX);
    ...
    function preRaceParticipantLimit_() {
        const visibleTotal = Number(visibleRaceDriverSlots_()?.total);
        if (Number.isFinite(visibleTotal) && visibleTotal > 0) {
            const total = Math.floor(visibleTotal);
            if (total !== preRaceSummaryStats.maxDrivers) {
                preRaceSummaryStats.maxDrivers = total;
                preRaceSummaryStats.driversComplete = preRaceSummaryStats.lastDrivers >= total;
            }
        }
        return preRaceSummaryStats.maxDrivers;
    }
    ...
    function cacheDirectRacingDataParticipantRows_(inputRows) {
        ...
        for (const source of rows) {
            ...
            const normalizedName = normalizeDriverName_(name);
            if (!driverId && !normalizedName) continue;
            const key = driverId ? `id:${driverId}` : `name:${normalizedName}`;
            const previous = byDriver.get(key) || {};
            byDriver.set(key, {
                name: name || previous.name || (driverId ? `Driver ${driverId}` : "Driver"),
                driverId: driverId || previous.driverId || "",
                racingSkill: row.racingSkill !== null && row.racingSkill !== undefined && row.racingSkill !== "" && Number.isFinite(Number(row.racingSkill))
                    ? Number(row.racingSkill)
                    : previous.racingSkill,
                itemID: row.itemID || previous.itemID || "",
                car: toOgCarName_(row.car || previous.car || ""),
                carImg: String(row.carImg || previous.carImg || ""),
                preRaceOnly: true,
                source: "racingData.carInfo"
            });
        }
        const nextRows = Array.from(byDriver.values());
        const key = nextRows.map(d => `${normalizeDriverName_(d.name)}:${d.driverId}:${d.itemID}:${d.car}`).sort().join("|");
        if (key !== directRacingDataParticipantsKey) {
            const previousById = new Map(preRaceParticipants.map(row => [String(row.driverId || ""), row]).filter(([id]) => id));
            const previousByName = new Map(preRaceParticipants.map(row => [normalizeDriverName_(row.name), row]).filter(([name]) => name));
            directRacingDataParticipants = nextRows;
            directRacingDataParticipantsKey = key;
            directRacingDataParticipantsById = new Map();
            directRacingDataParticipantsByName = new Map();
            for (const row of nextRows) {
                if (row.driverId) directRacingDataParticipantsById.set(String(row.driverId), row);
                const normalizedName = normalizeDriverName_(row.name);
                if (normalizedName) directRacingDataParticipantsByName.set(normalizedName, row);
            }
            preRaceParticipants = nextRows.map(row => {
                const previous = (row.driverId && previousById.get(String(row.driverId))) || previousByName.get(normalizeDriverName_(row.name)) || {};
                return Object.assign({}, previous, row, {
                    racingSkill: Number.isFinite(row.racingSkill) ? row.racingSkill : previous.racingSkill,
                    driverIntel: previous.driverIntel || row.driverIntel
                });
            });
            preRaceParticipantsKey = `racingData:${key}`;
            return true;
        }
        return false;
    }
    ...
    function applyPreRaceSummary_(summary, source = "pre-race", sequence = ++preRaceSummaryWorkerSequence) {
        ...
        const raceId = String(summary.raceId || "").trim();
        const raceIdChanged = !directCurrentRaceId && !!raceId;
        if (!directCurrentRaceId && raceId) directCurrentRaceId = raceId;
        preRaceSummaryStats.raceIdCaptured = !!directCurrentRaceId;
        const driversScanned = summary.driversScanned !== false;
        ...
        preRaceSummaryStats.timingReady = preRaceSummaryStats.timingReady || summary.timingReady === true;
        const maxDrivers = preRaceParticipantLimit_();
        if (maxDrivers > 0 && preRaceSummaryStats.lastDrivers >= maxDrivers) preRaceSummaryStats.driversComplete = true;
        const summaryLaps = Math.max(0, Number(summary.laps || 0) || 0);
        lastHeavyRaceStats = {
            drivers: preRaceSummaryStats.lastDrivers,
            laps: summaryLaps,
            intervals: 0,
            estimatedPoints: 0,
            heavyRace: preRaceSummaryStats.lastDrivers >= HEAVY_RACE_DRIVER_THRESHOLD || summaryLaps >= HEAVY_RACE_LAP_THRESHOLD
        };
        perfRecord_("capture.preRace.summary", 0, {
        ...
        return !!(raceId || preRaceSummaryStats.lastDrivers);
    }
    ...
    async function capturePreRaceSummaryText_(text, source, url) {
        ...
        try {
            ...
            const summary = await parsePreRaceSummaryOffMain_(text, !preRaceSummaryStats.driversComplete);
            applyPreRaceSummary_(summary, source, sequence);
            if (summary?.timingReady === true || summary?.preRace === false) {
                await yieldToMain_();
                return maybeAcceptRaceDataPayload_(JSON.parse(String(text || "")), source, url);
            }
            return !!(directCurrentRaceId || preRaceSummaryStats.lastDrivers);
        } catch (error) {
            if (debugEnabled) console.warn(TAG, "pre-race carInfo summary parse failed", source, error);
            return false;
        } finally {
            perfRecord_("capture.preRace.parse", perfNow_() - started, { source: String(source || ""), bytes: String(text || "").length, url: String(url || "") });
        }
    }
    ...
    function capturePreRaceSummaryObject_(raw, source, url = "") {
        ...
        const sequence = ++preRaceSummaryWorkerSequence;
        preRaceParticipantLimit_();
        const summary = preRaceSummaryFromObject_(raw, !preRaceSummaryStats.driversComplete);
        applyPreRaceSummary_(summary, source, sequence);
        if (summary.timingReady === true || summary.preRace === false) return maybeAcceptRaceDataPayload_(raw, source, url);
        return !!(directCurrentRaceId || preRaceSummaryStats.lastDrivers);
    }
    ...
    function findDirectRacingDataParticipant_(name) {
        const nn = normalizeDriverName_(name);
        if (!nn) return null;
        return directRacingDataParticipantsByName.get(nn) || null;
    }

    function mergeDirectRacingDataIntoParticipant_(d) {
        const directId = String(d?.driverId || "").trim();
        const direct = (directId && directRacingDataParticipantsById.get(directId)) || findDirectRacingDataParticipant_(d?.name || "");
        if (!direct) return d;
    ...
    async function fetchDirectRaceData_(raceId = "", source = "direct-racingData") {
        ...
        if (directRaceDataFetchComplete_(requestKey)) {
            const cached = boundedMapGet_(raceDataPayloadCacheByRaceId, requestKey);
    ...
    async function fetchDirectPreRaceSummary_(source = "direct-pre-race-summary") {
        const url = racingDataUrl_("");
        const resp = await fetchWithoutRaceDataCapture_(url, {
            credentials: "include",
            cache: "no-store",
            headers: {
        ...
        if (!resp.ok) return false;
        const text = await resp.text();
        if (!text || /^Wrong RFC/i.test(text.trim())) return false;
        return await capturePreRaceSummaryText_(text, source, url);
    }
    ...
    async function maybeFetchRacingDataForCurrentRace_(force = false) {
        ...
        const preRace = !replay && !visualRaceHasStarted_();
        if (preRace) refreshPreRaceParticipants_(force);

        if (!preRace && !analysis && !latestRaceDataPayload) await maybeWarmRestoreRaceDataForCurrentRace_();
        ...
        if (!preRace && directRaceDataFetchComplete_(resolvedRaceKey)) return false;
        const latestNativeDelivery = preRace ? preRaceSummaryReceivedPerfMs : raceDataReceivedPerfMs;
        const nativeRecentlyDelivered = latestNativeDelivery && (performance.now() - latestNativeDelivery < 8000);
        if (!force && !replay && nativeRecentlyDelivered) return false;
        const intervalMs = preRace ? 10000 : (replay ? (hasRaceData ? 10000 : 3000) : (hasRaceData ? 30000 : 15000));
        ...
        try {
            if (preRace) return await fetchDirectPreRaceSummary_("direct-current-pre-race");
    ...
    function maybeAcceptRaceDataPayload_(raw, source, url = "") {
        ...
        try {
            ...
            if (isCurrentEndpoint) {
                ...
                if (!directCurrentRaceId && payloadRid) directCurrentRaceId = payloadRid;
                preRaceSummaryStats.raceIdCaptured = !!directCurrentRaceId;
                if (!preRaceSummaryStats.driversComplete) {
                    cacheDirectRacingDataParticipants_(payload);
                    preRaceSummaryStats.lastDrivers = directRacingDataParticipants.length;
                    preRaceParticipantLimit_();
                }
            }
            const pageRid = getRaceId_();
            ...
            if (isCurrentEndpoint && usableForAnalysis) {
                preRaceSummaryStats.timingReady = true;
                preRaceSummaryStats.timingAccepted = true;
                preRaceSummaryStats.driversComplete = true;
            }
    ...
    function installPageRaceDataBridge_() {
        ...
        try {
            ...
            script.textContent = `(() => {
                ...
                const payloadIsPreRace = data => {
                    try {
                        const outer = data && typeof data === "object" ? data : {};
                        const time = outer.timeData || outer.raceData?.timeData || {};
                        const started = Number(time.timeStarted);
                        const current = Number(time.currentTime);
                        if (Number.isFinite(started) && Number.isFinite(current)) return current < started;
                        return !/race\\s+(?:started|finished|ended)/i.test(String(outer.info || outer.raceData?.info || ""));
                    } catch { return true; }
                };
                ...
                const emitPreRace = (data, source, url) => {
                    try {
                        const summary = summarizePreRace(data);
                        if (!summary || (!summary.raceId && !summary.participants.length)) return;
                        document.dispatchEvent(new CustomEvent(EVENT, {
                            detail: JSON.stringify({ source, url: String(url || ""), summary })
                        }));
                    } catch {}
                };
                try {
                        ...
                        NativeXHR.prototype.send = function() {
                            this.addEventListener("load", function() {
                                try {
                                    ...
                                    if (isEmptyRaceIdUrl(url) && payloadIsPreRace(data)) emitPreRace(data, "page-xhr-text-pre-race", url);
                                    else emit(data, "page-xhr-text", url);
                                } catch {}
                            });
                    ...
                    p.then(resp => {
                        try {
                            ...
                            if (shouldCapturePreRaceSummary_(url)) {
                                resp.clone().text().then(text => capturePreRaceSummaryText_(text, "fetch-pre-race", url)).catch(() => {});
                                return;
                            }
                            resp.clone().json().then(json => {
                ...
                NativeXHR.prototype.send = function () {
                    this.addEventListener("load", () => {
                        try {
                            ...
                            if (shouldCapturePreRaceSummary_(url)) {
                                if (this.response && typeof this.response === "object") {
                                    capturePreRaceSummaryObject_(this.response, "xhr-object-pre-race", url);
                                    perfRecord_("capture.preRace.object", perfNow_() - parseStarted, { responseType: String(this.responseType || "object") });
                                    return;
                                }
                                const preRaceText = String(this.responseText || "");
                                if (preRaceText) capturePreRaceSummaryText_(preRaceText, "xhr-pre-race", url).catch(() => {});
                                return;
                            }
                            if (this.response && typeof this.response === "object") {
    ...
    function buildAnalysisFromRaceData_(payload) {
        ...
        analysis = {
            ...
            processedSummary: boundedMapGet_(bigRaceProcessedSummaryCache, bigRaceIdentity.key) || null,
            segmentsPerLap,
            lapMeters,
            engineModel,
    ...
    function replayStateBucket_(elapsedSeconds, canFull) {
        if (canFull) return "full";
        const fps = liveRenderFps_();
        if (fps <= 0) return Math.floor((Number(elapsedSeconds) || 0) / 2);
        return Math.floor((Number(elapsedSeconds) || 0) * fps);
    }

    function currentReplayState_(elapsedSeconds = getVisualElapsed_(), canFull = analysisCanShowFullRace_()) {
        ...
        if (PHASE_REWRITE_FLAGS.replayStateSnapshots && replayStateCache.key === key && replayStateCache.value) {
            return replayStateCache.value;
        }
        const ordered = getLiveOrder_(full ? Infinity : elapsed, full);
        ...
        const state = {
            ...
            ordered,
            leader,
            leaderDriver: leader?.driver || null,
            leaderState: leader?.state || null,
            fieldSize: analysis.drivers.length,
            heavy: heavyRaceMode_(),
            finished: full || raceIsFinished_() || liveVisualHasEnded_()
        };
    ...
    function analysisAggregateKey_(model) {
        if (!model?.drivers?.length) return "";
        return [
            model.raceId || model.trackName || "race",
            model.bigRaceStaticSignature || "",
    ...
    function analysisAggregateDriverKey_(driver, index = 0) {
        return String(driver?.driverId || driver?.engineKey || normalizeDriverName_(driver?.name || "") || `driver-${index + 1}`);
    }

    function analysisAggregateInput_(model, options = {}) {
        if (!model?.drivers?.length) return null;
        return {
            key: analysisAggregateKey_(model),
            options: {
            ...
            meta: {
                ...
                laps: Number(model.laps || 0),
                segmentsPerLap: Number(model.segmentsPerLap || 0),
                lapMeters: Number(model.lapMeters || 0),
                largeFieldThreshold: LARGE_FIELD_THRESHOLD,
                hugeFieldThreshold: HUGE_FIELD_THRESHOLD
            },
            drivers: model.drivers.map((d, index) => ({
                key: analysisAggregateDriverKey_(d, index),
                name: String(d.name || ""),
                crashed: !!d.crashed,
                finalTime: Number(d.finalTime || 0),
                segmentTimes: (d.segmentTimes || []).map(Number),
                cumulativeTimes: (d.cumulativeTimes || []).map(Number),
                cumulativeDistances: (d.cumulativeDistances || []).map(Number),
                segmentDistancesMeters: (d.segmentDistancesMeters || []).map(Number),
                sectorTimes: (d.sectorTimes || []).map(s => ({
                    lap: Number(s.lap || 0),
...
function firstIndexAtOrAfter(arr,value){let lo=0,hi=(arr&&arr.length||0)-1,ans=(arr&&arr.length||0);while(lo<=hi){const mid=(lo+hi)>>1;if(value<=arr[mid]){ans=mid;hi=mid-1;}else lo=mid+1;}return ans;}
function distanceAtTime(d,t,segmentsPerLap){const times=d.cumulativeTimes||[],dists=d.cumulativeDistances||[],segTimes=d.segmentTimes||[],segDists=d.segmentDistancesMeters||[];if(!times.length)return{distance:0,finished:false,lapIndex:0};const finalTime=times[times.length-1]||0;if(finalTime>0&&t>=finalTime)return{distance:dists[dists.length-1]||0,finished:true,lapIndex:Math.floor(Math.max(0,times.length-1)/Math.max(1,segmentsPerLap||1))};const i=firstIndexAtOrAfter(times,Math.max(0,t||0));if(i<times.length){const prevTime=i?times[i-1]:0,prevDist=i?dists[i-1]:0,segTime=Math.max(.001,segTimes[i]||0),pct=Math.max(0,Math.min(1,(t-prevTime)/segTime));return{distance:prevDist+pct*(segDists[i]||0),finished:false,lapIndex:Math.floor(i/Math.max(1,segmentsPerLap||1))};}return{distance:dists[dists.length-1]||0,finished:true,lapIndex:Math.floor(Math.max(0,times.length-1)/Math.max(1,segmentsPerLap||1))};}
function reachTime(d,meters){const dists=d.cumulativeDistances||[],times=d.cumulativeTimes||[],segTimes=d.segmentTimes||[],segDists=d.segmentDistancesMeters||[];const i=firstIndexAtOrAfter(dists,meters);if(i<dists.length){const prevDist=i?dists[i-1]:0,prevTime=i?times[i-1]:0,segDist=Math.max(.001,segDists[i]||0),pct=Math.max(0,Math.min(1,(meters-prevDist)/segDist));return prevTime+pct*(segTimes[i]||0);}return Infinity;}
function aggregate(input){const drivers=(input&&input.drivers||[]).map(d=>Object.assign({},d));const meta=input&&input.meta||{};const rows=new Map();drivers.forEach(d=>rows.set(String(d.key),{key:String(d.key),sectorsWon:0,lapsLed:0,segmentsLed:0,timeInLeadSeconds:0,longestLeadStintSeconds:0,leadChanges:0}));if(input&&input.options&&input.options.sectors){const best=new Map();drivers.forEach(d=>{(d.sectorTimes||[]).forEach(s=>{const key=String(s.lap||0)+":"+String(s.sector||0);const seconds=Number(s.seconds);if(!Number.isFinite(seconds))return;const prev=best.get(key);if(!prev||seconds<prev.seconds)best.set(key,{driver:d,seconds});});});best.forEach(entry=>{const row=rows.get(String(entry.driver.key));if(row)row.sectorsWon=(row.sectorsWon||0)+1;});}if(input&&input.options&&input.options.leads){const maxTime=Math.max.apply(null,drivers.map(d=>Number(d.finalTime)||0).concat([0]));const count=drivers.length||0;const step=count>=Number(meta.hugeFieldThreshold||45)?5:(count>=Number(meta.largeFieldThreshold||30)?2:.5);let lastLeader=null,stint=0;for(let t=0;t<maxTime;t+=step){const slice=Math.min(step,Math.max(0,maxTime-t));let leader=null,leaderDistance=-Infinity;drivers.forEach(d=>{const state=distanceAtTime(d,t,meta.segmentsPerLap);const distance=state.distance||0;if(distance>leaderDistance){leaderDistance=distance;leader=d;}});if(!leader)continue;const row=rows.get(String(leader.key));if(row)row.timeInLeadSeconds=(row.timeInLeadSeconds||0)+slice;if(leader!==lastLeader){if(lastLeader){const prev=rows.get(String(lastLeader.key));if(prev)prev.longestLeadStintSeconds=Math.max(prev.longestLeadStintSeconds||0,stint);}if(row)row.leadChanges=(row.leadChanges||0)+1;lastLeader=leader;stint=0;}stint+=slice;}if(lastLeader){const row=rows.get(String(lastLeader.key));if(row)row.longestLeadStintSeconds=Math.max(row.longestLeadStintSeconds||0,stint);}for(let lap=1;lap<=Number(meta.laps||0);lap++){const meters=lap*(Number(meta.lapMeters)||0);let lapLeader=null,lapLeaderTime=Infinity;drivers.forEach(d=>{const time=reachTime(d,meters);if(Number.isFinite(time)&&time<lapLeaderTime){lapLeaderTime=time;lapLeader=d;}});if(lapLeader){const row=rows.get(String(lapLeader.key));if(row)row.lapsLed=(row.lapsLed||0)+1;}}const totalSegments=Number(meta.segmentsPerLap||0)*Number(meta.laps||0);for(let seg=1;seg<=totalSegments;seg++){let segmentLeader=null,segmentLeaderTime=Infinity;drivers.forEach(d=>{const time=(d.cumulativeTimes||[])[seg-1];if(Number.isFinite(time)&&time<segmentLeaderTime){segmentLeaderTime=time;segmentLeader=d;}});if(segmentLeader){const row=rows.get(String(segmentLeader.key));if(row)row.segmentsLed=(row.segmentsLed||0)+1;}}}return{key:input&&input.key||"",sectors:!!(input&&input.options&&input.options.sectors),leads:!!(input&&input.options&&input.options.leads),drivers:Array.from(rows.values())};}
self.onmessage=event=>{try{self.postMessage({ok:true,result:aggregate(event.data)});}catch(error){self.postMessage({ok:false,error:String(error&&error.message||error||"aggregate worker failed")});}};`;
    }

    function applyAnalysisAggregateResult_(model, result) {
        if (!model?.drivers?.length || !result || result.key !== analysisAggregateKey_(model)) return false;
        const byKey = new Map((result.drivers || []).map(row => [String(row.key || ""), row]));
        model.drivers.forEach((d, index) => {
            const row = byKey.get(analysisAggregateDriverKey_(d, index));
            if (!row) return;
            if (result.sectors) d.sectorsWon = Number(row.sectorsWon || 0);
            if (result.leads) {
                d.lapsLed = Number(row.lapsLed || 0);
                d.segmentsLed = Number(row.segmentsLed || 0);
                d.timeInLeadSeconds = Number(row.timeInLeadSeconds || 0);
                d.longestLeadStintSeconds = Number(row.longestLeadStintSeconds || 0);
                d.leadChanges = Number(row.leadChanges || 0);
            }
        });
        if (result.sectors) model.sectorWinsReady = true;
        if (result.leads) model.leadStatsReady = true;
        const body = document.getElementById("mpgAnalysisBody");
        if (body) body.dataset.renderKey = "";
        markDirty_({ data: true, status: true });
    ...
    function scheduleAnalysisAggregateWorker_(model, options = {}) {
        if (!PHASE_REWRITE_FLAGS.workerAggregates || !model?.heavyRace || !model?.drivers?.length) return false;
        if (typeof Worker === "undefined" || typeof Blob === "undefined" || typeof URL === "undefined" || typeof URL.createObjectURL !== "function") return false;
        const input = analysisAggregateInput_(model, options);
        if (!input?.key) return false;
        if (aggregateWorkerInFlightKey === input.key) return true;
        aggregateWorkerInFlightKey = input.key;
        aggregateWorkerLastStatus = "building";
        aggregateWorkerLastError = "";
        const started = perfNow_();
        let worker = null;
        let url = "";
        ...
        const finish = result => {
            ...
            try { if (worker) worker.terminate(); } catch {}
            try { if (url) URL.revokeObjectURL(url); } catch {}
            aggregateWorkerLastMs = Math.round((perfNow_() - started) * 100) / 100;
            aggregateWorkerInFlightKey = "";
            if (result?.ok && result.result) {
                aggregateWorkerLastStatus = "ready";
                aggregateWorkerFailedKey = "";
                perfRecord_("analysis.aggregates.worker", aggregateWorkerLastMs, { drivers: model.drivers.length, heavy: true });
                applyAnalysisAggregateResult_(model, result.result);
            } else {
                aggregateWorkerLastStatus = "failed";
                aggregateWorkerLastError = String(result?.error || "aggregate worker failed");
                aggregateWorkerFailedKey = input.key;
                if (debugEnabled) console.warn(TAG, aggregateWorkerLastError);
                perfRecord_("analysis.aggregates.worker_failed", aggregateWorkerLastMs, { drivers: model.drivers.length, heavy: true });
            }
            markDirty_({ status: true });
            scheduleRender_();
        };
        try {
            url = URL.createObjectURL(new Blob([buildAggregateWorkerSource_()], { type: "text/javascript" }));
            worker = new Worker(url);
            const timer = setTimeout(() => finish({ ok: false, error: "aggregate worker timed out" }), 8000);
            worker.onmessage = event => {
                clearTimeout(timer);
                finish(event.data || {});
            };
            worker.onerror = error => {
                clearTimeout(timer);
                finish({ ok: false, error: String(error?.message || error || "aggregate worker failed") });
            };
            worker.postMessage(input);
            return true;
        } catch (error) {
            finish({ ok: false, error: String(error?.message || error || "aggregate worker unavailable") });
            return false;
        }
    }
    ...
    function computeLeadStats_(model) {
        ...
        for (let t = 0; t < maxTime; t += step) {
            const slice = Math.min(step, Math.max(0, maxTime - t));
            let leader = null;
            ...
            if (!leader) continue;
            leader.timeInLeadSeconds += slice;
    ...
    function ensureAnalysisAggregates_(options = {}) {
        if (!analysis?.drivers?.length) return false;
        const wantsSectors = !!options.sectors;
        const wantsLeads = !!options.leads;
        ...
        if (needsSectors) {
            perfMeasure_("analysis.sectors.sync", () => computeSectorWins_(analysis), {
                drivers: analysis.drivers.length,
                heavy: !!analysis.heavyRace
            });
        }
        if (needsLeads) {
            perfMeasure_("analysis.leads.sync", () => computeLeadStats_(analysis), {
                drivers: analysis.drivers.length,
                laps: analysis.laps || 0,
                heavy: !!analysis.heavyRace
            });
        }
        if (analysis.heavyRace && analysis.leadStatsReady && analysis.sectorWinsReady) persistBigRaceProcessedSummary_(analysis);
        return true;
    }
    ...
    function addDriverIndexRow_(index, driverId, name, row) {
        for (const key of driverLookupKeys_(driverId, name)) {
            if (!index.has(key)) index.set(key, []);
            index.get(key).push(row);
        }
    }

    function uniqueDriverRows_(rows) {
        const seen = new Set();
        const out = [];
        for (const row of rows || []) {
            if (!row) continue;
            const key = row.id || row.lastRaceId || row.raceId || `${row.driverId || ""}|${row.driverName || ""}|${row.lastRaceAtIso || row.atIso || ""}|${row.car || row.lastCar || ""}`;
    ...
    function ensureDriverHistoryIndex_() {
        if (!driverHistoryLoaded) return driverHistoryByDriverIndex;
        if (!driverHistoryIndexDirty) return driverHistoryByDriverIndex;
        const index = new Map();
        for (const row of Object.values(driverHistory || {})) {
            if (!row || typeof row !== "object") continue;
            addDriverIndexRow_(index, row.driverId, row.driverName, row);
        }
        for (const rows of index.values()) {
            rows.sort((a, b) => String(b.lastRaceAtIso || b.updatedAt || "").localeCompare(String(a.lastRaceAtIso || a.updatedAt || "")));
        }
        driverHistoryByDriverIndex = index;
        driverHistoryIndexDirty = false;
        return driverHistoryByDriverIndex;
    }

    function getDriverHistoryRows_(driverId, name) {
        const index = ensureDriverHistoryIndex_();
        const rows = [];
        for (const key of driverLookupKeys_(driverId, name)) {
            const hit = index.get(key);
            if (hit?.length) rows.push(...hit);
        }
        return uniqueDriverRows_(rows)
            .sort((a, b) => String(b.lastRaceAtIso || b.updatedAt || "").localeCompare(String(a.lastRaceAtIso || a.updatedAt || "")));
    }

    function ensureRecordsIndex_() {
        if (!recordsLoaded) return recordsByDriverIndex;
        if (!recordsIndexDirty) return recordsByDriverIndex;
        const index = new Map();
        ...
        for (const rows of index.values()) {
            rows.sort((a, b) => String(b.atIso || "").localeCompare(String(a.atIso || "")));
        }
        recordsByDriverIndex = index;
        recordsIndexDirty = false;
        return recordsByDriverIndex;
    }

    function getRecordRowsForDriver_(driverId, name) {
        const index = ensureRecordsIndex_();
        const rows = [];
        for (const key of driverLookupKeys_(driverId, name)) {
            const hit = index.get(key);
            if (hit?.length) rows.push(...hit);
    ...
    function getDriverAnyHistory_(driverId, name, trackName = "") {
        ...
        if (exact) return exact;
        const rows = getDriverHistoryRows_(driverId, name);
        return rows[0] || null;
    }
    ...
    function getLastKnownDriverCar_(driverId, name, trackName = "") {
        ...
        const rows = getRecordRowsForDriver_(driverId, name).filter(r => r?.car);
        const row = rows.find(r => trackScope && getRecordTrackScopeFromRow_(r, r.mode || "race") === trackScope) || rows[0];
    ...
    function scrapeVisibleParticipants_() {
        ...
        const byKey = new Map();
        for (const direct of directRacingDataParticipants) {
            const key = direct.driverId ? `id:${direct.driverId}` : `name:${normalizeDriverName_(direct.name)}`;
            if (key && key !== "name:") byKey.set(key, { ...direct });
        }
        const large = largeLobbyMode_();
        const limit = visibleParticipantCandidateLimit_();
        ...
        for (const el of visibleParticipantCandidates_()) {
            ...
            const indexed = findDirectRacingDataParticipant_(name);
            const id = extractDriverIdFromElement_(el) || indexed?.driverId || (large ? "" : lookupVisibleDriverIdByName_(name, el));
            ...
            const prev = byKey.get(key) || indexed || {};
            byKey.set(key, {
                name: prev.name || name || "",
    ...
    function predictionDriverSet_() {
        if ((useApiPredictions && !driverIntelCacheLoaded) || (useHistoryPredictions && (!driverHistoryLoaded || !recordsLoaded))) {
            scheduleDeferredStartupCaches_("predictions");
        }
...
return {
    ...
    function getDriverIntelPool_(sourceDrivers = null) {
        const byId = new Map();
        const large = Array.isArray(sourceDrivers) ? sourceDrivers.length >= LARGE_FIELD_THRESHOLD : largeLobbyMode_();
        const add = d => {
...
(function(){
  ...
  window.addEventListener("resize",function(){
    ...
    resizeTimer=setTimeout(function(){renderPositionChart();renderTelemetryChart();},150);
  },{passive:true});
  function parse(v){
    v=(v||"").trim();
   ...
   * ============================ */
    ...
    function renderSettingsPanel_() {
        ...
        panel.querySelector("#mpgClearIntel").onclick = () => { driverIntelCache = {}; driverIntelCacheLoaded = true; saveDriverIntelCache_(); driverIntelStatus = "Driver Intel cache cleared."; if (analysis?.drivers) analysis.drivers.forEach(d => { d.driverIntel = null; }); uiDirty = true; scheduleRender_(); };
    ...
    async function deleteRecordFromButton_(btn) {
        ...
        if (source === "sqlite") {
            ...
            ensureRecordsLoaded_("deleteRecord");
    ...
    function renderAnalysis_() {
        const renderPerfStart = perfNow_();
        try {
        ...
        let aggregatesReady = true;
        if (canFull && analysisMode === "sectors") aggregatesReady = ensureAnalysisAggregates_({ sectors: true });
        if (canFull && (analysisMode === "summary" || analysisMode === "driver")) {
            aggregatesReady = ensureAnalysisAggregates_({ sectors: true, leads: true });
        }
        if (status) {
        ...
        return true;
            ...
            perfRecord_("render.analysis", perfNow_() - renderPerfStart, {
                mode: analysisMode,
                heavy: heavyRaceMode_(),
                rows: fieldSize_(),
                dataDirty,
    ...
    function renderSummaryMode_(elapsed = getVisualElapsed_(), canFull = null) {
        ...
        const leadWin = aggregatesReady ? focusedOrderWindow_(analysis.drivers.slice().sort((a,b)=>(b.timeInLeadSeconds||0)-(a.timeInLeadSeconds||0))) : { items: [], windowed: false };
        ...
        return `${allowPreFinishPreview && !raceIsFinished_() ? `<div class="mpg-note">Preview mode — uses full race data already delivered to page.</div>` : ""}${aggregateNote}${focusedWindowNote_(win, "Summary")}${renderTable_(["Pos","Car image + Car name","Driver (RS)","Status","Total","Best Lap","Ideal Lap","Top Speed","Highest Est. G"], rows, "Waiting for summary data...", "Summary", { windowed: win.windowed })}${leadWin.windowed ? focusedWindowNote_(leadWin, "Lead History") : ""}${renderTable_(["Driver (RS)","Time in Lead","Laps Led","Segments Led","Longest Stint","Lead Changes"], leadRows, aggregatesReady ? "No lead history yet." : "Lead history is still calculating.", "Lead History", { windowed: leadWin.windowed })}`;
    }
    ...
    function renderDriverStatsMode_(elapsed = getVisualElapsed_(), canFull = null) {
        ...
        const note = canFull
            ? `<div class="mpg-note">${aggregatesReady ? "" : "Heavy-race sector and lead aggregates are still calculating. "}Consistency is a 0–100 stability score based on completed lap variation: 100 − (lap-time standard deviation ÷ average lap × 100). Higher means more even lap times; lower means the driver's laps varied more.</div>`
    ...
    function ensureUi_() {
      ...
      `;
            ...
            window.addEventListener("resize", () => keepWindowInBounds_(w, "win", { persist: false }), { passive: true });
   ...
   * ============================ */
    ...
    function scheduleRender_(dirty = null) {
        ...
        const run = () => {
            ...
            requestAnimationFrame(() => {
                renderScheduled = false;
                perfMeasure_("render.total", render_, () => ({
                    mode: analysisMode,
                    heavy: heavyRaceMode_(),
                    rows: fieldSize_()
                }));
        ...
        if (PHASE_REWRITE_FLAGS.renderCoalescing && RENDER_COALESCE_MS > 0) {
            renderCoalesceTimer = setTimeout(run, RENDER_COALESCE_MS);
        } else {
            run();
        }
    }
    ...
    function render_() {
        ...
        if (recordsOpen && activeRecTbody && activeRecTrackSel && activeRecModeSel && !recordsLoaded) {
            scheduleDeferredStartupCaches_("records-panel");
            activeRecTbody.innerHTML = `<tr><td colspan="9" class="muted">Loading saved records...</td></tr>`;
        } else if (recordsOpen && activeRecTbody && activeRecTrackSel && activeRecModeSel) {
   ...
   * ============================ */
    async function init_() {
        ...
        ensurePlayer_();
        refreshSpectate_();
        scheduleDeferredStartupCaches_("init");
        if (apiKey) setTimeout(() => checkApiKeyInfo_({ manual: false }), 1000);
        scheduleInitialRaceDataFetch_();
