(() => {
  ...
  function createTimelineEntry(commit, index, total) {
    ...
    const time = createElement('time', '', `${timeFormatter.format(new Date(commit.committedAt))} UTC`);
  ...
  function renderFeed(feed, branch, commits, sourceLabel) {
    ...
    const panel = createElement('div', 'glass-panel rounded-lg flex h-[34rem] min-h-0 flex-col xl:h-full');
  ...
  async function start() {
    ...
    if (cached) {
      ...
      renderFeed(feed, cached.branch || fallbackBranch, cached.commits, isFresh ? 'CACHED' : 'STALE CACHE');
    }
    ...
    try {
      ...
      console.warn('Live repository activity could not be loaded; preserving the fallback feed.', error);
    } finally {
