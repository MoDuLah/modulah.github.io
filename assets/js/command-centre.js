...
function sanitiseMarketplace(value) {
  ...
  const appliesTo = value.appliesTo.map((site) => {
    if (!site || typeof site.name !== 'string' || !/^[a-z0-9.-]{1,253}$/.test(site.name))
      return null;
...
function sanitiseTimelineEvent(value) {
  if (!value || typeof value !== 'object') return null;
  if (typeof value.scriptId !== 'string' || !/^[A-Za-z0-9_-]{1,80}$/.test(value.scriptId))
    return null;
  if (typeof value.title !== 'string' || !value.title.length || value.title.length > 140)
    return null;
  if (
    typeof value.version !== 'string' ||
    !/^[0-9A-Za-z][0-9A-Za-z.+_-]{0,63}$/.test(value.version)
...
async function loadScriptUpdates() {
  ...
  try {
    ...
    if (payload?.schemaVersion !== 1 || !Array.isArray(payload.scripts))
      throw new Error('Invalid release feed');

    const updates = new Map(
      payload.scripts.filter(isSafeScriptUpdate).map((update) => [update.id, update])
    );
    ...
    if (status) {
      status.textContent =
        'Live update data could not be loaded; showing the built-in catalogue until the next visit.';
    }
    return false;
  } finally {
...
async function loadScreenshotManifest() {
  ...
  try {
    ...
    const payload = await response.json();
    if (payload?.schemaVersion !== 1 || !payload.modules || typeof payload.modules !== 'object')
      return false;
...
async function runRegistryTerminalSequence(
  ...
  return announceAndRenderModules(terminal, grid, filteredScripts, renderToken, sequenceToken);
}
...
function renderScriptUpdateTimeline() {
  ...
  releases.forEach((release, index) => {
    ...
    item.className =
      'relative flex gap-3 pb-5 before:absolute before:left-[5px] before:top-5 before:bottom-0 before:w-px before:bg-white/10 last:pb-0 last:before:hidden feed-item';
    ...
    if (release.href.startsWith('https://greasyfork.org/')) {
      const changelogLink = document.createElement('a');
      changelogLink.href = release.href;
      changelogLink.textContent = 'Read changelog';
      changelogLink.className = 'mt-2 inline-block text-[10px] text-[#98f05f] hover:underline';
      changelogLink.target = '_blank';
      changelogLink.rel = 'noopener noreferrer';
      content.append(changelogLink);
    }
...
function renderModuleFaq(data) {
  ...
  entries.forEach((entry) => {
    ...
    summary.className =
      'cursor-pointer list-none p-4 flex items-start justify-between gap-3 text-on-surface hover:text-[#98f05f] transition-colors';
    ...
    icon.className =
      'material-symbols-outlined text-[18px] shrink-0 group-open:rotate-180 transition-transform';
    icon.textContent = 'expand_more';
    summary.append(question, icon);
    const answer = document.createElement('p');
    answer.className =
      'mx-4 mb-4 pt-3 border-t border-white/5 text-on-surface-variant text-code-sm leading-relaxed';
...
function showScreenshot(index) {
  ...
  document.getElementById('screenshot-dialog-position').textContent =
    `SCREENSHOT ${activeScreenshotIndex + 1} / ${activeScreenshots.length}`;
...
function renderModuleScreenshots(data) {
  ...
  const screenshots = (
    discoveredModuleScreenshots[data.id] ||
    moduleScreenshots[data.id] ||
    []
  ).map((screenshot) => ({
    ...screenshot,
  ...
  if (!screenshots.length) {
    ...
    empty.className =
      'sm:col-span-2 xl:col-span-3 border border-dashed border-outline-variant rounded p-6 text-center text-code-sm text-outline-variant';
...
function createModuleCard(script, index) {
  ...
  title.className =
    'font-headline-lg-mobile text-headline-lg-mobile text-on-surface mb-1 group-hover:text-[#98f05f] transition-colors';
  ...
  if (script.marketplace) {
    ...
    marketplace.className =
      'font-code-sm text-[10px] text-[#98f05f]/70 mt-3 uppercase tracking-wider';
  ...
  footer.className =
    'module-card-reveal mt-4 pt-4 border-t border-white/5 font-code-sm text-code-sm text-[#98f05f]/80 flex justify-between';
  const version = document.createElement('span');
  version.textContent = `${script.versionPrefix}: ${script.version}`;
  const arrow = document.createElement('span');
  arrow.setAttribute('aria-hidden', 'true');
  arrow.className =
    'material-symbols-outlined text-[16px] opacity-0 group-hover:opacity-100 transition-opacity';
...
function renderModuleFacts(data) {
  ...
  if (data.marketplace) {
    facts.push(
      ...
      {
        label: 'License',
        value: data.marketplace.license.name,
        href: data.marketplace.license.url,
      },
...
function renderModuleActions(data) {
  ...
  (data.disabledActions || []).forEach((label) => {
    ...
    disabled.className =
      'font-label-caps text-label-caps uppercase px-6 py-3 rounded-DEFAULT border border-outline-variant text-outline-variant opacity-60 cursor-not-allowed flex items-center justify-center gap-2';
...
function populateModuleDetail(data) {
  ...
  document.getElementById('detail-version').textContent = `${data.versionPrefix}: ${data.version}`;
...
function getModuleDiagnosticSteps(data) {
  ...
  const disabledActions = Array.isArray(data.disabledActions) ? data.disabledActions.length : 0;
  const steps = [
    {
...
async function unmountModuleDetail(grid, details, selectedCard) {
  ...
  if (!selectedCard || reducedMotion) {
    ...
    grid.addEventListener('animationend', () => grid.classList.remove('registry-return-fallback'), {
      once: true,
    });
    return;
  }
